...
| `nem_life_protection_assessment_flow`   | APPROVED       | `/protection-check/start` assessment shell                  | `code.html`               | Desktop top bar, right protection map rail, glass action card, progress concept. Keep natural scroll available and derive completed rail areas from answered questions, not visual position. HTML contains remote assets and prototype-only Tailwind; do not paste. |
